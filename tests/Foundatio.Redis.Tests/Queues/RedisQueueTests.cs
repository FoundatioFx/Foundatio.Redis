...
{
    private readonly string _lockTopic = $"test-queue-{Guid.NewGuid().ToString("N")[..10]}";
    ...
    public override async Task CanDequeueWithLockingAsync()
    {
        ...
        using var messageBus = new RedisMessageBus(new RedisMessageBusOptions { Subscriber = muxer.GetSubscriber(), Topic = _lockTopic, LoggerFactory = Log });
